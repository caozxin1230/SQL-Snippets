...
LEFT JOIN corp_actions_listing cal ON cal.vendor_event_id=ca.vendor_event_id
LEFT JOIN override_event_distribution_cash_flow_set ocfs ON ocfs.vendor_event_id=ca.vendor_event_id
WHERE cal.ex_date>'2013-12-31'AND ca.source_id='TK' AND ca.event_type='DVC' and cfs.default_cash_flow='N' AND cfs.composition_type='2'
AND ca.event_id IS NOT NULL AND ca.status='A') AS cf_2
...
AND ca.event_id IS NOT NULL AND ca.status='A') AS cf_3

ON cf_2.vendor_event_id != cf_3.vendor_event_id AND cf_1.vendor_event_id!= cf_3.vendor_event_id AND cf_2.cash_flow_set_id=cf_3.cash_flow_set_id OR cf_3.parent_cfs_id=cf_2.parent_cfs_id


ORDER BY cf_2.ex_date DESC
